      ...
      spec {
        ...
        container {
          ...
          env {
            ...
            value = "PLAINTEXT://kafkaservice.twitterpipeline.svc.cluster.local:9092,PLAINTEXT_INTERNAL://kafkaservice.${kubernetes_namespace.pipeline-namespace.metadata.0.name}.svc.cluster.local:29092"
          }
        ...
        container {
          ...
          args = [
            "-c",
            "kafka-topics --bootstrap-server kafkaservice:29092 --list; echo -e 'Creating kafka topics'; kafka-topics --bootstrap-server kafkaservice:29092 --create --if-not-exists --topic tweets --replication-factor 1 --partitions 1; echo -e 'Successfully created the following topics:'; kafka-topics --bootstrap-server kafkaservice:29092 --list; tail -f /dev/null;",
          ]
